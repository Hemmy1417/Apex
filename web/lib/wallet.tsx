...
const CONNECTED_KEY = "apex_connected_rdns";
...
export function WalletProvider({ children }: { children: React.ReactNode }) {
  ...
  const refreshBalance = useCallback(async () => {
    if (!client || !address) { setBalanceWei(null); return; }
    try {
  ...
  const disconnect = useCallback(() => {
    setAddress(""); setClient(null); setBalanceWei(null);
  ...
  useEffect(() => {
    ...
    w.provider
      ...
      .then((accs: string[]) => { if (accs?.[0]) bind(accs[0], w.provider); })
      .catch(() => {});
  ...
  const connect = useCallback(
    async (w?: Discovered) => {
      ...
      try {
        ...
        } catch { /* declined or already added */ }
  ...
  return (
    <Ctx.Provider value={{ address, client, connecting, wallets, hasWallet: wallets.length > 0, chainName: CHAIN_NAME, gasSponsored: GAS_SPONSORED, balanceWei, refreshBalance, connect, disconnect }}>
      {children}
    </Ctx.Provider>
  );
}
